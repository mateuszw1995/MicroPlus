...
namespace db {
	namespace network {
		...
		struct overlapped : public db::threads::overlapped {
			DWORD flags;
			overlapped();
		};

		class overlapped_accept : public db::threads::overlapped {
			friend class tcp;
			SOCKET accepted;
		...
		class tcp {
			friend class db::threads::iocp;
			friend class overlapped_accept;
			friend bool db::network::init();
			SOCKET sock;
